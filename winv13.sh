...
if [[ "$choice" == "y" ]]; then
    ...
    if [[ "$ROOTLESS" == "1" ]]; then
        ...
        if [[ "$OS_ID" == "ubuntu" ]]; then
            LLVM_VER=21
            spin_start "Cài LLVM ${LLVM_VER} (Ubuntu)..."
            export DEBIAN_FRONTEND=noninteractive
            install -d -m 0755 /etc/apt/keyrings
            curl -fsSL https://apt.llvm.org/llvm-snapshot.gpg.key | gpg --dearmor -o /etc/apt/keyrings/apt.llvm.org.gpg
            UBUNTU_CODENAME="${UBUNTU_CODENAME:-$(. /etc/os-release && echo "${UBUNTU_CODENAME:-${VERSION_CODENAME:-$(lsb_release -cs 2>/dev/null || echo jammy)}}") }"
            UBUNTU_CODENAME="${UBUNTU_CODENAME// /}"
            echo "deb [signed-by=/etc/apt/keyrings/apt.llvm.org.gpg] http://apt.llvm.org/${UBUNTU_CODENAME}/ llvm-toolchain-21 main" > /etc/apt/sources.list.d/llvm-toolchain-21.list
            apt-get update -qq
            apt-get install -y -qq clang-21 lld-21 llvm-21 llvm-21-dev llvm-21-tools
