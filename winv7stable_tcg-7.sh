...
if [[ "$choice" == "y" ]]; then
    if [ -x /opt/qemu-optimized/bin/qemu-system-x86_64 ]; then
        ...
        for f in $(grep -rl "define TB_JMP_CACHE_BITS" include/ accel/ tcg/ 2>/dev/null); do
...
-DNDEBUG -DDEFAULT_TCG_TB_SIZE=3145728 -DCONFIG_TCG_INTERPRETER=0"
        ...
        # --tools: chỉ build qemu-img, bỏ qemu-nbd/qemu-io/qemu-storage-daemon
        # Giảm compile time đáng kể so với build full
        ../qemu-src/configure \
