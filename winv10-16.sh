...
_rootless_build() {
    ...
    # ── Python 3.12 từ conda (nhanh hơn build từ source) ─────────
    echo -ne "${B}◜${W} Cài Python 3.12 từ conda..."
    conda install -y -q python=3.12 > /tmp/conda-python.log 2>&1
    if [[ $? -eq 0 ]]; then
        echo -e "\r${G}✔${W} Python 3.12 từ conda xong          "
    else
        echo -e "\r${R}✘${W} conda Python thất bại — xem /tmp/conda-python.log"
        exit 1
    fi

    # PY_PREFIX trỏ vào conda env
    ...
    conda install -y -q -c conda-forge meson ninja pkg-config \
    ...
    # ── pixman ───────────────────────────────────────────────────
    echo -ne "${B}◜${W} Build pixman..."
    cd "$BUILD"
    wget -c -qO- https://cairographics.org/releases/pixman-0.43.4.tar.gz | tar xz > /dev/null 2>&1
    cd pixman-0.43.4
    ...
    meson setup build --prefix="$PREFIX" --default-library=static \
        -Dtests=disabled > /tmp/pixman-build.log 2>&1
    ninja -C build install >> /tmp/pixman-build.log 2>&1
    echo -e "\r${G}✔${W} pixman xong          "

    # ── libffi ───────────────────────────────────────────────────
    echo -ne "${B}◜${W} Build libffi..."
    cd "$BUILD"
    wget -c -qO- https://github.com/libffi/libffi/releases/download/v3.4.6/libffi-3.4.6.tar.gz | tar xz > /dev/null 2>&1
    cd libffi-3.4.6
    CFLAGS="-fPIC" ./configure --prefix="$PREFIX" \
        --enable-static --disable-shared --with-pic > /tmp/libffi-build.log 2>&1
    make -j"$(nproc)" install >> /tmp/libffi-build.log 2>&1
    echo -e "\r${G}✔${W} libffi xong          "

    # ── pcre2 ────────────────────────────────────────────────────
    echo -ne "${B}◜${W} Build pcre2..."
    cd "$BUILD"
    wget -c -qO- https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.44/pcre2-10.44.tar.gz | tar xz > /dev/null 2>&1
    cd pcre2-10.44
    CFLAGS="-fPIC" ./configure --prefix="$PREFIX" \
        --enable-static --disable-shared --with-pic > /tmp/pcre2-build.log 2>&1
    make -j"$(nproc)" install >> /tmp/pcre2-build.log 2>&1
    echo -e "\r${G}✔${W} pcre2 xong          "

    # ── glib 2.80 ────────────────────────────────────────────────
    echo -ne "${B}◜${W} Tải glib 2.80.0..."
    cd "$BUILD"
    wget -c -qO- https://download.gnome.org/sources/glib/2.80/glib-2.80.0.tar.xz | tar xJ > /dev/null 2>&1
    echo -e "\r${G}✔${W} Tải glib 2.80.0 xong          "

    # Pre-populate pcre2 subproject cache để glib không tự download
    echo -ne "${B}◜${W} Chuẩn bị pcre2 subproject cho glib..."
    mkdir -p "$BUILD/glib-2.80.0/subprojects/packagecache"
    wget -c -q \
        "https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.42/pcre2-10.42.tar.gz" \
        -O "$BUILD/glib-2.80.0/subprojects/packagecache/pcre2-10.42.tar.gz" \
        > /dev/null 2>&1 || true
    # Tạo wrap file để meson dùng .tar.gz thay vì .tar.bz2
    cat > "$BUILD/glib-2.80.0/subprojects/pcre2.wrap" << 'WRAPEOF'
[wrap-file]
directory = pcre2-10.42
source_url = https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.42/pcre2-10.42.tar.gz
source_filename = pcre2-10.42.tar.gz
source_hash = 8d36cd8cb6ea2a4c2bb358ff6411b0c788633a2a45dabbf1aeb4b701d1b5e840
WRAPEOF
    echo -e "\r${G}✔${W} pcre2 subproject cache xong          "

    echo -ne "${B}◜${W} Configure glib 2.80.0..."
    cd "$BUILD/glib-2.80.0"
    GLIB_BUILD_OK=0
    meson setup build \
        --prefix="$PREFIX" \
        ...
        -Dsysprof=disabled \
        -Dxattr=false \
        > /tmp/glib-configure.log 2>&1 && \
    ninja -C build install > /tmp/glib-build.log 2>&1 && \
    GLIB_BUILD_OK=1

    if [[ "$GLIB_BUILD_OK" == "1" ]]; then
        echo -e "\r${G}✔${W} glib 2.80.0 xong          "
    else
        echo -e "\r${Y}⚠${W}  glib build từ source thất bại — thử conda..."
        if command -v conda &>/dev/null; then
            conda install -y -q -c conda-forge glib > /tmp/glib-conda.log 2>&1 \
                && echo -e "${G}✔${W} glib từ conda xong" \
                || { echo -e "${R}✘${W} conda glib cũng thất bại — xem /tmp/glib-conda.log"; exit 1; }
            # Link pkg-config glib từ conda vào PREFIX
            CONDA_GLIB_PC=$(find "$CONDA_PREFIX" -name "glib-2.0.pc" 2>/dev/null | head -1)
            if [[ -n "$CONDA_GLIB_PC" ]]; then
                export PKG_CONFIG_PATH="$(dirname "$CONDA_GLIB_PC"):$PKG_CONFIG_PATH"
                echo -e "${G}✔${W} PKG_CONFIG_PATH cập nhật từ conda"
            fi
        else
            echo -e "${R}✘${W} Không có conda — không thể fallback"
            echo "   Xem log: /tmp/glib-configure.log"
            echo "   Xem log: /tmp/glib-build.log"
            exit 1
        fi
    fi
    ...
    # Thêm conda include/lib vào extra flags
    CONDA_INC="${CONDA_PREFIX:-/opt/conda}/include"
    CONDA_LIB="${CONDA_PREFIX:-/opt/conda}/lib"
    QEMU_EXTRA_CFLAGS="-I$PREFIX/include -I$CONDA_INC"
    QEMU_EXTRA_LDFLAGS="-L$PREFIX/lib64 -L$PREFIX/lib -L$CONDA_LIB"
