...
# QEMU 10.2.1 TCG + LLVM Extreme Build (V12 - Fixed & Optimized)
# Improvements in V12:
#   - Added qemu-utils to dependencies (Fixes qemu-img command not found)
#   - Added qemu-img to the build target to ensure latest version is available
#   - Automatically adds /opt/qemu-llvm/bin to PATH for the current session
#   - Improved dependency check for faster startup
...
if [[ "$choice" != "y" ]]; then
    ...
    if [ -x /opt/qemu-llvm/bin/qemu-system-x86_64 ]; then
        ...
        echo "[!] QEMU not found in /opt/qemu-llvm/bin. Installing qemu-utils as fallback..."
        sudo apt update && sudo apt install -y qemu-utils
    fi
else
if [ -x /opt/qemu-llvm/bin/qemu-system-x86_64 ] && [ -x /opt/qemu-llvm/bin/qemu-img ]; then
    echo "[OK] QEMU LLVM & Tools already installed"
    export PATH="/opt/qemu-llvm/bin:$PATH"
else

echo "=== [1/6] Dependencies ==="
sudo apt update
# Added qemu-utils here to fix the "command not found" error
...
ulimit -n 65535 || true
# Build both the system emulator and qemu-img
...
echo "BUILD COMPLETE (V12)"
/opt/qemu-llvm/bin/qemu-system-x86_64 --version
fi
fi

#=========================================================================
# VM MANAGER (V12 Fixed)
#=========================================================================
# Ensure qemu-img is available even if build was skipped
if ! command -v qemu-img &> /dev/null; then
    if [ -x /opt/qemu-llvm/bin/qemu-img ]; then
        export PATH="/opt/qemu-llvm/bin:$PATH"
    else
