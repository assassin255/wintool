...
echo "Downloading $WN..."
[[ ! -f win.img ]] && silent aria2c -x16 -s16 --continue --file-allocation=none "$WU" -o win.img
read -rp "Expand disk GB (default 20): " eg; eg="${eg:-20}"
silent qemu-img resize win.img "+${eg}G"
...
BO=""; [[ "$UE" == "yes" ]] && BO="-bios /usr/share/qemu/OVMF.fd"
...
echo "Starting VM: tb=${TS}KB aio=$AIO cores=$cc ram=${rs}G jemalloc=$([ -n "$JP" ] && echo y || echo n) hugepages=$([ -n "$MP" ] && echo y || echo n)"

env $JP qemu-system-x86_64 \
...
sleep 3
use_rdp=$(ask "Open RDP tunnel? (y/n): " "n")
if [[ "$use_rdp" == "y" ]]; then
    silent wget -q https://github.com/kami2k1/tunnel/releases/latest/download/kami-tunnel-linux-amd64.tar.gz
    silent tar -xzf kami-tunnel-linux-amd64.tar.gz && silent chmod +x kami-tunnel
    sudo apt install -y tmux -qq 2>/dev/null
    tmux kill-session -t kami 2>/dev/null || true
    tmux new-session -d -s kami "./kami-tunnel 3389"; sleep 4
    PUB=$(tmux capture-pane -pt kami -p | sed 's/\x1b\[[0-9;]*m//g' | grep -i public | grep -oE '[a-zA-Z0-9\.\-]+:[0-9]+' | head -1)
    echo ""
    echo "========================================"
    echo "  VM DEPLOYED"
    echo "  OS: $WN | CPU: ${cc}c | RAM: ${rs}G | TB: ${TS}KB | AIO: $AIO"
    echo "  RDP: $PUB | User: $RU | Pass: $RP"
    echo "========================================"
fi
