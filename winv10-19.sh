...
detect_hugepage() {
    ...
    if [ "$HP_1G" -gt 0 ]; then
        ...
        if [ "$HP_1G" -lt "$NEED" ]; then
            _try_write "$NEED" /sys/kernel/mm/hugepages/hugepages-1048576kB/nr_hugepages
        ...
        if [ "$HP_2M" -lt "$NEED" ]; then
            _try_write "$NEED" /sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages
        ...
        if _try_write "always" /sys/kernel/mm/transparent_hugepage/enabled && grep -q "always" /sys/kernel/mm/transparent_hugepage/enabled 2>/dev/null; then
            THP="always"
        ...
        if echo "$THP" | grep -q '\[always\]\|always'; then
            # Tối ưu thêm THP
            _try_write "defer+madvise" /sys/kernel/mm/transparent_hugepage/defrag
...
#  KHỞI ĐỘNG VM
# ════════════════════════════════════════════════════════════════
# ── Kernel tweaks — chỉ chạy nếu có quyền ───────────────────
_try_write() { _try_write "$1" "$2"; }

if [[ "$(id -u)" == "0" ]] || sudo -n true 2>/dev/null; then
    _try_write 0       /proc/sys/kernel/watchdog
    _try_write 10      /proc/sys/vm/swappiness
    _try_write "always" /sys/kernel/mm/transparent_hugepage/enabled
    _try_write "defer+madvise" /sys/kernel/mm/transparent_hugepage/defrag
    echo -e "${G}✔${W} Kernel tweaks: watchdog off, swappiness=10, THP=always"
else
    echo -e "${B}ℹ${W}  Bỏ qua kernel tweaks (không có quyền root/sudo)"
fi
