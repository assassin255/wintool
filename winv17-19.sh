...
PY
    ...
    if [[ "$use_rdp" == "y" && -x "$BORE_BIN" ]]; then
        ...
        rm -f "$BORE_PID_FILE" "$BORE_URL_FILE"

        # ── 4. Khởi động bore tunnel ──────────────────────────────
        echo -e "${B}ℹ${W}  Khởi động bore tunnel → ${BORE_RELAY}:3389..."
        : > "$BORE_LOG"
        nohup "$BORE_BIN" local 3389 \
...
if [[ -n "$PUBLIC" ]]; then
    ...
    echo -e "🛑 Stop tunnel  : ${Y}kill \$(cat ${BORE_PID_FILE})${W}"
else
    echo -e "📡 RDP (local)  : ${G}localhost:3389${W}"
    [[ "$use_rdp" == "y" ]] && \
        echo -e "${Y}   ⚠  Bore chưa lấy được endpoint — xem $BORE_LOG${W}"
fi
