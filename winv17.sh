...
spin_start() {
    ...
    # Braille dots — mượt hơn nhiều so với ◜◝◞◟
    local frames=('⣾' '⣽' '⣻' '⢿' '⡿' '⣟' '⣯' '⣷')
    _SPIN_START=$(date +%s)
    (
        local i=0
        while :; do
            ...
            printf "\r  ${B}%s${W} %s ${B}[%ds]${W}   " "$f" "$msg" "$elapsed"
            sleep 0.08
            (( i++ )) || true
        done
    ) &
...
_detect_kvm() {
    ...
    if [[ ! -e /dev/kvm ]]; then
        echo -e "${Y}⚠${W}  /dev/kvm không tồn tại — dùng LLVM"
        KVM_AVAILABLE=0
        KVM_MODE="tcg"
        return
    fi
    ...
    if [[ "$KVM_OWNER" == "root" ]] && [[ "$KVM_GROUP" == "root" || "$KVM_GROUP" == "kvm" ]]; then
        ...
        echo -e "   Phát hiện     : ${R}${KVM_OWNER}:${KVM_GROUP}${W} → fallback LLVM"
        CAN_USE_KVM=0
    fi
    ...
    if [[ $CAN_USE_KVM -eq 1 ]]; then
        ...
        if grep -qE '(vmx|svm)' /proc/cpuinfo 2>/dev/null; then
            ...
            echo -e "${Y}⚠${W}  Fallback sang LLVM JIT"
            KVM_AVAILABLE=0
            KVM_MODE="tcg"
        fi
    else
        echo -e "${Y}⚠${W}  Không đủ quyền dùng /dev/kvm — dùng LLVM"
        KVM_AVAILABLE=0
        KVM_MODE="tcg"
    fi
...
_rootless_build() {
    ...
    # ════════════════════════════════════════════════════════════════
    #  CONDA DETECTION + AUTO-INSTALL NẾU CHƯA CÓ
    # ════════════════════════════════════════════════════════════════
    HAVE_CONDA=0
    CONDA_INSTALL_FAILED=0

    _try_install_conda() {
        echo ""
        echo -e "${C}════════════════════════════════════${W}"
        echo -e "${C}📦 TẢI MINICONDA (rootless)${W}"
        echo -e "${C}════════════════════════════════════${W}"
        local CONDA_DIR="$HOME/miniconda3"
        local CONDA_SH="$HOME/miniconda_install.sh"

        spin_start "Tải Miniconda installer..."
        wget -q -O "$CONDA_SH" \
            "https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh" \
            > /tmp/conda-download.log 2>&1
        if [[ $? -ne 0 ]]; then
            spin_fail "Tải Miniconda thất bại"
            CONDA_INSTALL_FAILED=1
            return 1
        fi
        spin_stop "Tải Miniconda xong"

        spin_start "Cài Miniconda vào $CONDA_DIR..."
        bash "$CONDA_SH" -b -p "$CONDA_DIR" > /tmp/conda-install.log 2>&1
        if [[ $? -ne 0 ]]; then
            spin_fail "Cài Miniconda thất bại — xem /tmp/conda-install.log"
            CONDA_INSTALL_FAILED=1
            return 1
        fi
        spin_stop "Miniconda đã cài"
        ...
        if command -v conda &>/dev/null; then
            echo -e "${G}✔${W} Conda sẵn sàng: $(conda --version)"
            return 0
        else
            echo -e "${R}✘${W} Conda vẫn không khởi động được sau khi cài"
            CONDA_INSTALL_FAILED=1
            return 1
        fi
    }
    ...
    if command -v conda &>/dev/null; then
        HAVE_CONDA=1
        export PY_PREFIX="${CONDA_PREFIX:-$HOME/miniconda3}"
        export PATH="$PY_PREFIX/bin:$HOME/.local/bin:$PREFIX/bin:$PATH"
        echo -e "${G}✔${W} Phát hiện conda tại: ${CONDA_PREFIX:-$PY_PREFIX}"
    else
        echo -e "${Y}⚠${W}  Không phát hiện conda — thử tải Miniconda..."
        if _try_install_conda; then
            HAVE_CONDA=1
            export PY_PREFIX="${CONDA_PREFIX:-$HOME/miniconda3}"
            export PATH="$PY_PREFIX/bin:$HOME/.local/bin:$PREFIX/bin:$PATH"
        else
            echo -e "${Y}⚠${W}  Miniconda thất bại — dùng system python"
            HAVE_CONDA=0
    ...
    if [[ $HAVE_CONDA -eq 1 ]]; then
        ...
        if [[ -n "$PY_VER_CONDA" ]]; then
            ...
            echo -e "\r${Y}⚠${W} Python conda không detect được"
            HAVE_CONDA=0
        fi
    fi
    ...
    # ── Cài meson + ninja ────────────────────────────────────────
    if [[ $HAVE_CONDA -eq 1 ]]; then
        echo -ne "${B}◜${W} Cài meson/ninja/pkg-config từ conda..."
        if conda install -y -q -c conda-forge meson ninja pkg-config \
                > /tmp/conda-tools.log 2>&1; then
            echo -e "\r${G}✔${W} meson/ninja/pkg-config từ conda xong          "
        else
            echo -e "\r${Y}⚠${W} conda tools thất bại"
        fi
    fi
    ...
    cd "$BUILD"
    if [[ ! -d qemu-11.0.0 ]]; then
        wget -c -qO- https://download.qemu.org/qemu-11.0.0.tar.xz | tar xJ > /dev/null 2>&1
    fi
    echo -e "\r${G}✔${W} QEMU 11.0.0 sẵn sàng          "

    # ── Build PKG_CONFIG_PATH ───────────────────────────────────
    if [[ $HAVE_CONDA -eq 1 && -n "${CONDA_PREFIX:-}" ]]; then
        export PKG_CONFIG_PATH="${CONDA_PREFIX}/lib/pkgconfig:${CONDA_PREFIX}/lib64/pkgconfig:${PKG_CONFIG_PATH:-}"
    fi
    export PKG_CONFIG_PATH="$PREFIX/lib/pkgconfig:$PREFIX/lib64/pkgconfig:${PKG_CONFIG_PATH:-}"
    export LD_LIBRARY_PATH="$PREFIX/lib:$PREFIX/lib64:${CONDA_PREFIX:-}/lib:${CONDA_PREFIX:-}/lib64:${LD_LIBRARY_PATH:-}"

    echo -e "${B}ℹ${W}  PKG_CONFIG_PATH=$PKG_CONFIG_PATH"
    echo -e "${B}ℹ${W}  LD_LIBRARY_PATH=$LD_LIBRARY_PATH"
    ...
    # ── Configure vanilla QEMU ───────────────────────────────────
    echo -ne "${B}◜${W} Configure QEMU rootless..."
    cd "$BUILD/qemu-11.0.0"
    rm -rf build

    # Export environment cho configure
    export PKG_CONFIG_PATH
    export LD_LIBRARY_PATH
    export CC="${CC:-gcc}"
    export CXX="${CXX:-g++}"
    export LDFLAGS="-L$PREFIX/lib -L$PREFIX/lib64 -Wl,-rpath,$PREFIX/lib:$PREFIX/lib64:${CONDA_PREFIX:-}/lib"
    export CFLAGS="-I$PREFIX/include ${CONDA_PREFIX:+-I${CONDA_PREFIX}/include}"
    ...
    if [[ $? -ne 0 ]]; then
        echo -e "\r${R}✘${W} Configure QEMU thất bại"
        echo "=== Configure log ==="
        cat /tmp/qemu-configure.log | tail -30
        exit 1
    fi
    ...
    make -j"$(nproc)" 2>&1 | tee /tmp/qemu-build.log | grep --line-buffered -E "^\[|error:|warning:|FAILED" || true
    
    if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
        echo -e "${R}✘ Compile QEMU thất bại${W}"
        echo "=== Last 50 lines of build log ==="
        tail -50 /tmp/qemu-build.log
        exit 1
    fi

    echo -e "${G}✔ Compile QEMU thành công${W}"

    # ── Install ──────────────────────────────────────────────────
    echo -ne "${B}◜${W} Cài đặt QEMU..."
    make install > /dev/null 2>&1
    strip "$PREFIX/bin/qemu-system-x86_64" 2>/dev/null || true
    echo -e "\r${G}✔${W} QEMU đã cài vào $PREFIX          "

    export LD_LIBRARY_PATH="$PREFIX/lib:$PREFIX/lib64:${CONDA_PREFIX:-}/lib:${LD_LIBRARY_PATH:-}"
...
-DCONFIG_TCG_INTERPRETER=0"
        ...
        if [[ "$KVM_AVAILABLE" == "1" ]]; then
            ...
            QEMU_KVM_FLAG="--disable-kvm"
            echo -e "${B}ℹ${W}  QEMU apt-build: --disable-kvm (LLVM JIT mode)"
        fi
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    echo -e "${C}⚡ Acceleration: ${Y}LLVM JIT${C}${W}"
fi
echo -e "${C}════════════════════════════════════${W}"
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    # ── TCG MODE ─────────────────────────────────────────────────
    echo -e "${Y}⚡ VM sẽ chạy với LLVM JIT (software acceleration)${W}"
...
if [[ "$use_rdp" == "y" ]]; then
    ...
    if [[ "$KVM_AVAILABLE" == "1" ]]; then
        ...
        echo -e "⚡ Acceleration : ${Y}LLVM JIT | TB cache: ${TCG_TB_MB}MB${W}"
