...
_tcg_tune_root() {
    ...
    if echo performance | tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor \
            > /dev/null 2>&1; then
        echo -e "${G}✔${W} CPU governor → performance (tất cả cores)"
    else
        echo -e "${Y}⚠${W}  CPU governor: không ghi được"
    fi

    # ── 2. EPP → performance ────────────────────────────────────
    local epp_ok=0
    for p in /sys/devices/system/cpu/cpufreq/policy*/energy_performance_preference; do
        [[ -f "$p" ]] && echo performance > "$p" 2>/dev/null && epp_ok=1
    done
    [[ $epp_ok -eq 1 ]] \
        && echo -e "${G}✔${W} EPP → performance (Intel HWP)" \
        || echo -e "${Y}⚠${W}  EPP: không ghi được"
    ...
    if [[ "$nr_2m" -lt "$pages_needed" && -z "$QEMU_HUGEPAGES_DIR" ]]; then
        echo "$pages_needed" > /sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages 2>/dev/null
        local after
        ...
        if [[ "$after" -ge "$pages_needed" ]]; then
            ...
            echo -e "${Y}⚠${W}  Không allocate đủ hugepages (${after}/${pages_needed})"
        fi
    fi
    # THP → always
    ...
    if [[ -f "$sch_path" ]]; then
        echo mq-deadline > "$sch_path" 2>/dev/null \
            && echo -e "${G}✔${W} Disk scheduler → mq-deadline (${disk_dev})" \
            || echo -e "${Y}⚠${W}  Disk scheduler: không ghi được"
        # Read-ahead tăng lên 4MB
        echo 8192 > "/sys/block/${disk_dev}/queue/read_ahead_kb" 2>/dev/null \
