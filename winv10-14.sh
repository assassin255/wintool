...
_rootless_build() {
    ...
    ./configure --prefix="$PY_PREFIX" --with-ensurepip=install > /dev/null 2>&1
    ...
    spin_start "Cài pip packages..."
    "$PY_PREFIX/bin/python3" -m pip install -q --upgrade \
        pip setuptools wheel packaging meson ninja truststore > /dev/null 2>&1
    spin_stop "pip packages xong"
