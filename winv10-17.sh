...
_rootless_build() {
    ...
    # ── pkg-config wrapper cho rootless ──────────────────────────
    mkdir -p "$PREFIX/bin"
    cat > "$PREFIX/bin/pkg-config" << 'PKGCFG'
#!/bin/sh
exec /usr/bin/pkg-config "$@"
PKGCFG
    # Nếu hệ thống không có pkg-config, dùng pkgconf
    if ! command -v pkg-config &>/dev/null && command -v pkgconf &>/dev/null; then
        ln -sf "$(command -v pkgconf)" "$PREFIX/bin/pkg-config"
    elif ! command -v pkg-config &>/dev/null; then
        # Build pkg-config minimal từ source
        echo -ne "${B}◜${W} Build pkg-config..."
        cd "$BUILD"
        wget -c -qO- https://pkgconfig.freedesktop.org/releases/pkg-config-0.29.2.tar.gz | tar xz > /dev/null 2>&1
        cd pkg-config-0.29.2
        ./configure --prefix="$PREFIX" --with-internal-glib > /tmp/pkgconfig-build.log 2>&1
        make -j"$(nproc)" install >> /tmp/pkgconfig-build.log 2>&1
        echo -e "\r${G}✔${W} pkg-config xong          "
    fi
    ...
    local gcfg=$?
    if [[ $gcfg -ne 0 ]]; then
        echo -e "\r${R}✘${W} Configure glib thất bại — xem /tmp/glib-configure.log"
        exit 1
    fi
    echo -e "\r${G}✔${W} Configure glib xong          "

    echo -ne "${B}◜${W} Build glib 2.80.0 (mất vài phút)..."
    if ninja -C build install > /tmp/glib-build.log 2>&1; then
        echo -e "\r${G}✔${W} glib 2.80.0 xong          "
    else
        echo -e "\r${R}✘${W} glib build thất bại — xem /tmp/glib-build.log"
        exit 1
    fi
