...
if [[ "$choice" == "y" ]]; then
    if [ -x /opt/qemu-optimized/bin/qemu-system-x86_64 ]; then
        ...
        echo "🔧 Applying TCG source patches..."

        # ============================================================
        # PATCH 1: TCG_MAX_INSNS 512 → 1024
        # Số guest instruction tối đa trong 1 Translation Block trước
        # khi TCG buộc phải kết thúc TB. Tăng lên giảm số lần exit
        # TB dispatcher → ít overhead hơn. QEMU upstream đã test 1024.
        # ============================================================
        TCG_C="tcg/tcg.c"
        if grep -q "define TCG_MAX_INSNS" "$TCG_C" 2>/dev/null; then
            sed -i "s/#define TCG_MAX_INSNS [0-9]*/#define TCG_MAX_INSNS 1024/" "$TCG_C"
            echo "  ✅ PATCH 1: TCG_MAX_INSNS → 1024"
        else
            echo "  ⚠️ PATCH 1: TCG_MAX_INSNS không tìm thấy trong $TCG_C, bỏ qua"
        fi

        # ============================================================
        # PATCH 2: TCG_HIGHWATER 1024 → 2048
        # Highwater là ngưỡng buffer TCG còn lại trước khi force-flush.
        # Tăng lên giảm số lần flush sớm, cho phép emit code dài hơn.
        # Không vượt quá buffer size mặc định, safe.
        # ============================================================
        if grep -q "define TCG_HIGHWATER" "$TCG_C" 2>/dev/null; then
            sed -i "s/#define TCG_HIGHWATER [0-9]*/#define TCG_HIGHWATER 2048/" "$TCG_C"
            echo "  ✅ PATCH 2: TCG_HIGHWATER → 2048"
        else
            echo "  ⚠️ PATCH 2: TCG_HIGHWATER không tìm thấy, bỏ qua"
        fi

        # ============================================================
        # PATCH 3: TB_JMP_CACHE_BITS 12 → 13
        # Hash table tra cứu TB theo guest PC. Tăng từ 4096 → 8192
        # entries giảm hash collision khi workload nhiều code path.
        # Chỉ tốn thêm ~64KB RAM, không ảnh hưởng correctness.
        # ============================================================
        find include/ accel/ tcg/ -type f \( -name "*.h" -o -name "*.c" \) 2>/dev/null             | xargs grep -l "define TB_JMP_CACHE_BITS" 2>/dev/null             | while read f; do
                sed -i "s/#define TB_JMP_CACHE_BITS [0-9]*/#define TB_JMP_CACHE_BITS 13/" "$f"
                echo "  ✅ PATCH 3: TB_JMP_CACHE_BITS → 13 in $f"
            done
        grep -rq "define TB_JMP_CACHE_BITS" include/ accel/ tcg/ 2>/dev/null             || echo "  ⚠️ PATCH 3: TB_JMP_CACHE_BITS không tìm thấy, bỏ qua"

        # ============================================================
        # PATCH 4: cpu-exec.c — tối ưu lookup_tb_ptr hot path
        # Thêm __builtin_expect hint cho branch thường gặp nhất:
        # TB hit (cached) xảy ra >> TB miss. Giúp compiler xếp code
        # đúng thứ tự predict → giảm branch misprediction penalty.
        # Chỉ patch nếu chưa có hint (tránh double-patch).
        # ============================================================
        CPU_EXEC="accel/tcg/cpu-exec.c"
        if [ -f "$CPU_EXEC" ] && grep -q "tb_lookup" "$CPU_EXEC" 2>/dev/null; then
            # Thêm __builtin_expect vào điều kiện tb != NULL nếu chưa có
            if ! grep -q "__builtin_expect" "$CPU_EXEC" 2>/dev/null; then
                sed -i "s/if (tb != NULL)/if (__builtin_expect(tb != NULL, 1))/g" "$CPU_EXEC"
                sed -i "s/if (tb == NULL)/if (__builtin_expect(tb == NULL, 0))/g" "$CPU_EXEC"
                echo "  ✅ PATCH 4: __builtin_expect hints vào cpu-exec.c"
            else
                echo "  ✅ PATCH 4: __builtin_expect đã có sẵn, bỏ qua"
            fi
        else
            echo "  ⚠️ PATCH 4: cpu-exec.c không tìm thấy hoặc không có tb_lookup, bỏ qua"
        fi

        # ============================================================
        # PATCH 5: tcg/tcg.c — tăng TCG_POOL_CHUNK_SIZE
        # Pool chunk dùng để alloc các TCG temp variable.
        # Tăng từ 32K → 64K giảm số lần realloc pool trong session dài.
        # ============================================================
        if grep -q "define TCG_POOL_CHUNK_SIZE" "$TCG_C" 2>/dev/null; then
            sed -i "s/#define TCG_POOL_CHUNK_SIZE ([0-9]* \* 1024)/#define TCG_POOL_CHUNK_SIZE (64 * 1024)/" "$TCG_C"
            echo "  ✅ PATCH 5: TCG_POOL_CHUNK_SIZE → 64KB"
        else
            echo "  ⚠️ PATCH 5: TCG_POOL_CHUNK_SIZE không tìm thấy, bỏ qua"
        fi

        echo "✅ TCG source patches done"
        ...
        #   -DNDEBUG                    tắt assert() trên TCG hot path
        #   -fno-strict-overflow        TCG cần wrap arithmetic, tắt UB assumption của compiler
        #   -fvisibility=hidden         giảm PLT thunk overhead cho TCG internal calls
...
-DNDEBUG -DDEFAULT_TCG_TB_SIZE=3145728 -DCONFIG_TCG_INTERPRETER=0"
            LDFLAGS="-flto=full -fuse-ld=lld -Wl,--lto-O3 -Wl,--gc-sections -Wl,--icf=all -Wl,-O3"
            LTO_OPT="--enable-lto"
        else
            EXTRA_CFLAGS="-O2 -march=native -mtune=native -pipe \
-fmerge-all-constants -fno-semantic-interposition \
